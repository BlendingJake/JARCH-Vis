...
class JVWindows(JVBuilderBase):
    ...
    @staticmethod
    def _update_mesh_from_geometry_lists(
        mesh, geometry_lists: List[Tuple[list, list, list]]
    ):
        """
    ...
    def _regular(props, mesh):
        width, height, jamb_w = (
            props.window_width_medium,
            props.window_height_medium,
            props.jamb_width,
        )
        frame_width, pane_th, jamb_th = (
            props.frame_width,
            props.frame_thickness,
            Units.INCH,
        )
        ...
        for _ in range(props.num_joined_windows):
            ...
            geometry_data.append(
                JVWindows._rectangular_jamb_geometry(
                    width, height, (x, 0, 0), jamb_w, keep_left=x == 0, th=jamb_th
                )
            )
            ...
            if (
                props.slider
            ):  # first pane is offset on y if there is going to be a second pane
                y = -pane_th / 2

            geometry_data.append(
                JVWindows._rectangular_pane_geometry(
                    pane_width,
                    pane_height,
                    (x + jamb_th, y, z),
                    frame_th=pane_th,
    ...
    def _arch(props, mesh):
        ...
        (
            ...
            width / 2,
            (width * roundness) / 2,
        )  # a, b for main window, need modified for jamb, inset frame, etc.
        frame_width, jamb_th, frame_th, inset = (
            props.frame_width,
            Units.INCH,
            props.frame_thickness,
            Units.Q_INCH,
    ...
    def _polygon(props, mesh):
        ...
        radius, jamb_w, jamb_th, sides = (
            props.window_radius,
            props.jamb_width,
            Units.INCH,
            props.window_side_count,
        )
        frame_width, frame_th, inset = (
            props.frame_width,
            props.frame_thickness,
            Units.Q_INCH,
        )  # pane variables
        ...
        steps = [  # one set of steps for the jamb, one set of steps for the pane
            (
                (radius, -hjamb_w),
                (radius + jamb_th, -hjamb_w),
                (radius + jamb_th, hjamb_w),
                (radius, hjamb_w),
            ),
            (
                (radius - frame_width, -hframe_th + inset),
                (radius - frame_width, -hframe_th),
                (radius, -hframe_th),
                (radius, hframe_th),
                (radius - frame_width, hframe_th),
                (radius - frame_width, hframe_th - inset),
    ...
    def _gothic(props, mesh):
        ...
        res, frame_width, frame_th = (
            props.window_resolution // 2,
            props.frame_width,
            props.frame_thickness,
        )
        jamb_th = Units.INCH

        hframe_th, hjamb_w, hwidth, inset = (
            frame_th / 2,
            jamb_w / 2,
            width / 2,
            Units.Q_INCH,
        ...
        vpl = (
            2 + (res + 2) + (res + 1)
        )  # two bottom, res + 2 on right side, res + 1 on left side
        level_z = height - hwidth * sqrt(
            3
        )  # the z value at which the arc begins (ignoring the thickness of the jamb)
        ...
        for substep in steps:
            ...
            for sxz, sy, r in substep:
                ...
                iterator = iter(
                    JVWindows._gothic_arc_iterator(r, cx, res, low_to_high=False)
                )
        ...
        # glass faces
        verts, faces, glass_ids = geometry_data[
            -1
        ]  # get last entry which will be the pane's geometry
        JVWindows._close_glass_faces_in_loops(len(verts), vpl, faces, glass_ids)
    ...
    def _ellipse(props, mesh):
        geometry_data = JVWindows._ellipse_worker(
            props.window_width_wide,
            props.window_height_short,
            props.jamb_width,
            props.frame_width,
            props.window_resolution,
    ...
    def _circular(props, mesh):
        ...
        jamb_th, frame_width, frame_th, inset = (
            Units.INCH,
            props.frame_width,
            props.frame_thickness,
            Units.Q_INCH,
        ...
        if props.full_circle:
            ...
            geometry_data = []
            start_angle, end_angle = ang - radians(180), -radians(
                180
            )  # start angle from -X axis
            hangle = (end_angle + start_angle) / 2

            steps = (
                (  # radius, y value  z value
                    (radius, -hjamb_w, jamb_th),
                    (radius + jamb_th, -hjamb_w, 0),
                    (radius + jamb_th, hjamb_w, 0),
                    (radius, hjamb_w, jamb_th),
                ),
                (
            ...
            for substep in steps:
                ...
                for rad, y, z in substep:
                    ...
                    cv = Vector(
                        (z / sin(abs(hangle)), 0, 0)
                    )  # adjust z so cv[2]=z and x ends up between whatever
                    cv.rotate(
                        Euler((0, hangle, 0))
                    )  # rotate center vector to proper place
    ...
    @staticmethod
    def _bow(props, mesh):
        jamb_w, frame_width, frame_th, jamb_th = (
            props.jamb_width,
            props.frame_width,
            props.frame_thickness,
            Units.INCH,
        )
        angle_per_pane, radius = (
            radians(180) / props.bow_segments,
            props.window_width_wide / 2,
        )
        pane_start_angle = (
            radians(180) - angle_per_pane
        ) / 2  # how much the pane is rotated from the radius
        pane_width, height = (
            2 * radius * sin(angle_per_pane / 2),
            props.window_height_medium,
        )
        inset = Units.Q_INCH
        ...
        geometry_data = []
        corner_vector, angle_step = Vector((-radius, 0, 0)), Euler(
            (0, 0, -angle_per_pane)
        )
        cur_pane_angle = pane_start_angle

        inner_jamb, outer_jamb = Vector((-pane_center_radius + hjw, 0, 0)), Vector(
            (-pane_center_radius - hjw, 0, 0)
        )
        ...
        for i in range(props.bow_segments):
            ...
            verts, faces, glass_ids = JVWindows._rectangular_pane_geometry(
                pane_width,
                height,
                (0, frame_th / 2, jamb_th),
                fw=frame_width,
                inset=inset,
                frame_th=frame_th,
            )

            JVWindows._transform_vertex_positions(
                verts,
                rotation=Euler((0, 0, cur_pane_angle)),
            ...
            geometry_data.append(
                (
                    *JVWindows._bay_bow_vertical_filler_strip(
                        corner_vector,
                        cur_pane_angle + radians(90),
    ...
    @staticmethod
    def _bay(props, mesh):
        width, height, pane_angle = (
            props.window_width_wide,
            props.window_height_medium,
            props.bay_angle,
        )
        depth = props.window_depth
        jamb_w, jamb_th, frame_w, frame_th = (
            props.jamb_width,
            Units.INCH,
            props.frame_width,
            props.frame_thickness,
        ...
        for angle, pw, corner in (
            ...
            JVWindows._transform_vertex_positions(
                verts, Euler((0, 0, angle)), after_translation=corner
            )
            geometry_data.append((verts, faces, glass_ids))
        ...
        # in between
        for x, angle in (
            (-large_width / 2, radians(90)),
            (large_width / 2, radians(90) - pane_angle),
        ):
            geometry_data.append(
                (
                    *JVWindows._bay_bow_vertical_filler_strip(
        ...
        for corner, inside_sign in (
            (Vector((-large_width / 2, depth, 0)), -1),
            (Vector((large_width / 2, depth, 0)), 1),
        ):
    ...
    def _ellipse_worker(
        ...
        steps = (
            ...
            (
                (-frame_width, -hframe_th + inset),
                (-frame_width, -hframe_th),
                (0, -hframe_th),
                (0, hframe_th),
                (-frame_width, hframe_th),
                (-frame_width, hframe_th - inset),
        ...
        for substep in steps:
            ...
            for ab_offset, yy in substep:
                ...
                # lower half
                for x, z in JVWindows._ellipse_iterator(
                    a + ab_offset, b + ab_offset, res
                ):
                    verts.append(
                        (x, yy, z)
                    )  # negative because points are generate in CCW order, so flip side

                # top half
                for x, y, z in verts[
                    -2:p:-1
                ]:  # get all except end two points in reverse order
                    verts.append((x, y, -z))
    ...
    @staticmethod
    def _rectangular_jamb_geometry(
        width: float,
        height: float,
        start_coord: tuple,
        jamb_width: float,
        ...
        for yy in (y - hj, y + hj):
            verts += [  # outer loop followed by inner loop
                (x, yy, z),
                (x + th + width + th, yy, z),
                (x + th + width + th, yy, z + th + height + th),
                (x, yy, z + th + height + th),
                (x + dx, yy, z + th),
                (x + th + width, yy, z + th),
                (x + th + width, yy, z + th + height),
                (x + dx, yy, z + th + height),
            ]
    ...
    @staticmethod
    def _rectangular_pane_geometry(
        w: float,
        h: float,
        start_coord: tuple,
        frame_th=1.5 * Units.INCH,
        inset=Units.Q_INCH,
        fw=Units.INCH,
    ) -> Tuple[list, list, list]:
        """
        ...
        fy, gy = (
            frame_th / 2,
            (frame_th - (2 * inset)) / 2,
        )  # half of the frame y and half of the glass y
    ...
    def _loop_face_builder(num_loops, vpl, faces):
        ...
        p, e = (
            0,
            (num_loops - 1) * vpl,
        )  # first vertex index and index of vertex at the start of the last loop
