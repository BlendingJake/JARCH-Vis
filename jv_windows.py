...
class JVWindows(JVBuilderBase):
    ...
    @staticmethod
    def _arch_function(half_width, half_height, res):
        """
        an iterator that returns an x, y coordinates for an ellipse based on the half-width and half-height of the ellipse
        """
    ...
    @staticmethod
    def _arch(props, verts, faces, mat_ids):
        width, height, jamb_w = props.window_width_medium, props.window_height_medium, props.jamb_width

        I, t = Units.INCH, 1.5 * Units.INCH
        bevel_y, bevel_xz = Units.H_INCH, Units.Q_INCH
        arch_h = height * (props.window_roundness / 100)
        arch_h2 = arch_h + I
        ...
        # jamb base
        verts += [
            (-hw - I, -w, 0.0), (hw + I, -w, 0.0), (hw + I, w, 0.0), (-hw - I, w, 0.0), (-hw, -w, I), (hw, -w, I),
            (hw, w, I), (-hw, w, I), (-hw - I, -w, h + I), (-hw, -w, h + I), (-hw, w, h + I), (-hw - I, w, h + I),
            (hw, -w, h + I), (hw + I, -w, h + I), (hw + I, w, h + I), (hw, w, h + I)
        ]

        faces += [
            (0, 3, 2, 1), (0, 1, 5, 4), (2, 3, 7, 6), (4, 5, 6, 7), (0, 4, 9, 8), (0, 8, 11, 3), (3, 11, 10, 7),
        ...
        # jamb arch
        res = props.window_resolution
        for i in JVWindows._arch_function(hw, arch_h, res):
            verts += [(i[0], -w, i[1] + h + I), (i[0], w, i[1] + h + I)]

        off = len(verts) - p
        for i in JVWindows._arch_function(hw2, arch_h2, res):
            verts += [(i[0], -w, i[1] + h + I), (i[0], w, i[1] + h + I)]

        # jamb faces
        for i in range(res - 2):  # arch faces
            faces += [
                (p, p + 1, p + 3, p + 2), (p, p + 2, p + off + 2, p + off),
        ...
        # connecting faces
        faces += [
            (p2 - 8, p2 - 7, p2, p2 + off), (p2 - 7, p2 - 6, p2 + 1, p2), (p2 - 6, p2 - 5, p2 + off + 1, p2 + 1),
            (p2 - 8, p2 + off, p2 + off + 1, p2 - 5), (p2 - 4, p2 - 3, p + off, p),
            (p2 - 3, p2 - 2, p + off + 1, p + off), (p2 - 2, p2 - 1, p + 1, p + off + 1), (p2 - 1, p2 - 4, p, p + 1)
        ]

        # if slide add extra pane
        sz, ph = I, h
        if props.slider:
            JVWindows._create_rectangle_pane(verts, faces, mat_ids, width, (h + I) / 2, 0.0, 0.0, I + (h + I) / 4,
                                  [0.0, 0.0, 0.0, 0.0])
            sz = (h + I) / 2
            ph = h - (h - I) / 2
            y = 0.0
        ...
        # border bottom
        verts += [
            (-hw, y, sz), (hw, y, sz), (hw, y + t, sz), (-hw, y + t, sz), (-hw + I, y, sz + I),
            (hw - I, y, sz + I), (hw - I, y + t, sz + I), (-hw + I, y + t, sz + I),
            (-hw + I + bevel_xz, y + bevel_y, sz + I + bevel_xz), (hw - I - bevel_xz, y + bevel_y, sz + I + bevel_xz),
            (hw - I, y + t - Units.ETH_INCH, sz + I), (-hw + I, y + t - Units.ETH_INCH, sz + I)
        ]
        ...
        p3 = len(verts)
        verts += [
            (-hw, y, sz), (hw, y, sz), (hw, y + t, sz), (-hw, y + t, sz), (-hw + I, y, sz), (hw - I, y, sz),
            (hw - I, y + t, sz), (-hw + I, y + t, sz), (-hw + I + bevel_xz, y + bevel_y, sz),
            (hw - I - bevel_xz, y + bevel_y, sz), (hw - I, y + t - Units.ETH_INCH, sz),
            (-hw + I, y + t - Units.ETH_INCH, sz)
        ]
        ...
        for i in JVWindows._arch_function(hw, arch_h, res):
            temp1 += [
                (i[0], y, i[1] + h + I), (i[0], y + t, i[1] + h + I)
            ]

        temp2 = []
        for i in JVWindows._arch_function(hw - I, arch_h - I, res):
            temp2 += [
                (i[0], y, i[1] + h + I), (i[0], y + t - Units.ETH_INCH, i[1] + h + I), (i[0], y + t, i[1] + h + I)
            ]

        temp3 = []
        for i in JVWindows._arch_function(hw - I - bevel_xz, arch_h - I - bevel_xz, res):
            temp3.append((i[0], y + bevel_y, i[1] + h + I))

        mat_ids += [len(faces) + 13, len(faces) + 14]
        # bottom and side faces for border
        faces += [
            (p, p + 1, p + 5, p + 4), (p + 4, p + 5, p + 9, p + 8), (p, p + 3, p + 2, p + 1),
            (p + 2, p + 3, p + 7, p + 6), (p + 6, p + 7, p + 11, p + 10), (p, p + 4, p + 16, p + 12),
            (p + 4, p + 8, p + 20, p + 16), (p + 1, p + 13, p + 17, p + 5), (p + 5, p + 17, p + 21, p + 9),
            (p + 2, p + 6, p + 18, p + 14), (p + 6, p + 10, p + 22, p + 18), (p + 11, p + 7, p + 19, p + 23),
        ...
        for i in range(res - 2):
            faces += [
                (p, p + 2, p + 8, p + 6), (p + 2, p + 3, p + 9, p + 8), (p + 4, p + 5, p + 11, p + 10),
                (p + 1, p + 7, p + 11, p + 5), (p, p + 1, p + 7, p + 6)
            ]
            p += 6

        mat_ids += [len(faces), len(faces) + 1]
        faces.append(inner_face)
        faces.append(outer_face)

        mat_ids += [len(faces) + 10, len(faces) + 11]
        # extra faces
        faces += [
            (p3, p3 + 4, p3 + 14, p3 + 12), (p3 + 4, p3 + 8, p3 + 15, p3 + 14),
            (p3 + 7, p3 + 3, p3 + 13, p3 + 17), (p3 + 11, p3 + 7, p3 + 17, p3 + 16), (p3, p3 + 3, p3 + 13, p3 + 12),
            (p3 + 1, p4 - 6, p4 - 4, p3 + 5), (p3 + 5, p4 - 4, p4 - 3, p3 + 9), (p3 + 6, p3 + 10, p4 - 2, p4 - 1),
            (p3 + 2, p3 + 6, p4 - 1, p4 - 5), (p3 + 1, p4 - 6, p4 - 5, p3 + 2), (p3 + 8, p3 + 9, p4 - 3, p3 + 15),
        ...
        if is_bay:  # bay
            ...
            for r in [bay_angle, -bay_angle]:
                ...
                if db_hung:
                    ...
                    create_rectangle_pane(verts, faces, glass_indices, side_pw, h, 0, 0.0, h / 2 + I,
                                          [HI, 0.0, 0.0, 0.0])
    ...
    def circular(radius, angle, res, jamb_w, full):
        ...
        if full:
            ...
            for i in range(res + 3):
                faces += [(p, p + off, p + off + 2, p + 2), (p, p + 2, p + 3, p + 1),
                          (p + off, p + off + 1, p + off + 3, p + off + 2),
                          (p + 1, p + 3, p + off + 3, p + off + 1)]
                p += 2

            faces += [(p, p + off, off, 0), (p, p + 1, 1, 0), (p + 1, 1, off + 1, p + off + 1),
                      (off, p + off, p + off + 1, off + 1)]
            p = len(verts)
            ...
            edge_ang = radians(90) - acos(
                2 * I / (radius - 2 * I))  # adjust angle to make room for next tier of vertices
            ...
            edge_ang = radians(90) - acos(
                (2 * I + bevelXZ) / (radius - 2 * I - bevelXZ))  # adjust angle for smaller radius
            ang = (angle - 2 * edge_ang) / res
            for i in range(res, -1, -1):
            ...
            for i in range(res + 3):
                faces += [(p, p + 2, p + 8, p + 6), (p + 2, p + 3, p + 9, p + 8), (p + 4, p + 5, p + 11, p + 10),
                          ...
                          (p, p + 1, p + 7, p + 6)]
                p += 6
            faces += [(p, p + 2, p2 + 2, p2), (p + 2, p + 3, p2 + 3, p2 + 2), (p2 + 1, p2 + 5, p + 5, p + 1),
                      (p2 + 5, p2 + 4, p + 4, p + 5)]
    ...
    @staticmethod
    def _create_rectangle_pane(verts, faces, mat_ids, width, height, sx, sy, sz, wide_frame):
        """
        ...
        :param sy: start y
        :param sz: start z
        :param wide_frame: wide_frame is extra [bottom, right, top, left]
        """
        ...
        for i2 in range(1, -2, -2):  # invert for top and bottom
            ...
            for i in range(-1, 2, 2):  # invert for sides
                ...
                verts += [
                    (sx + i * w, sy - t, sz - i2 * h), (sx + i * (w - I) + ex2, sy - t, sz - i2 * (h - I) + ex),
                    (sx + i * (w - I - bevel_xz) + ex2, sy - t + bevel_y, sz - i2 * (h - I - bevel_xz) + ex),
                    (sx + i * (w - I) + ex2, sy - t + bevel_y, sz - i2 * (h - I) + ex),
                    (sx + i * (w - I) + ex2, sy - Units.ETH_INCH, sz - i2 * (h - I) + ex),
                    (sx + i * (w - I) + ex2, sy, sz - i2 * (h - I) + ex), (sx + i * w, sy, sz - i2 * h)
                ]

        # faces
        faces += [
            (p + 0, p + 7, p + 8, p + 1), (p + 1, p + 8, p + 9, p + 2), (p + 6, p + 5, p + 12, p + 13)
        ]
        p += 14
        faces += [
            (p + 0, p + 1, p + 8, p + 7), (p + 1, p + 2, p + 9, p + 8), (p + 6, p + 13, p + 12, p + 5)
        ]
        p -= 14
        faces += [
            (p, p + 1, p + 15, p + 14), (p + 1, p + 2, p + 16, p + 15), (p, p + 14, p + 20, p + 6),
        ...
        faces += [
            (p + 2, p + 9, p + 23, p + 16), (p + 4, p + 18, p + 25, p + 11), (p, p + 6, p + 13, p + 7),
            (p + 14, p + 21, p + 27, p + 20), (p + 2, p + 3, p + 17, p + 16), (p + 3, p + 4, p + 17, p + 18),
            (p + 11, p + 10, p + 24, p + 25), (p + 10, p + 9, p + 23, p + 24), (p + 4, p + 5, p + 12, p + 11),
            (p + 18, p + 19, p + 26, p + 25)
        ]
    ...
    def gothic(width, height, res, is_slider, jamb_w):
        ...
        offsets = [[0, int(res / 2) + 1, -1, 1],
                   [int(res / 2) - 1, -1, 1, -1]]  # start, stop, mod1/angle side, mod2/step
        ...
        for i in range(res + 2):
            faces += [(p, p + off, p + off + 2, p + 2), (p + off, p + off + 1, p + off + 3, p + off + 2),
                      (p, p + 2, p + 3, p + 1),
                      (p + 1, p + 3, p + off + 3, p + off + 1)]
            p += 2
        faces += [(p, p + off, off, 0), (p, p + 1, 1, 0), (1, off + 1, p + off + 1, p + 1),
                  (off, p + off, p + off + 1, off + 1)]
        ...
        for g in offsets:
            for i in range(g[0], g[1], g[3]):
                x, z = point_rotation((g[2] * (hw - I), edge_height), (g[3] * (hw + I), edge_height),
                                      g[2] * ang_off * i)
                temp2 += [(x, y, z), (x, y + t - ei, z), (x, y + t, z)]
        temp2 += [(hw - I, y, 2 * I + h_off), (hw - I, y + t - ei, 2 * I + h_off), (hw - I, y + t, 2 * I + h_off)]
        ...
        for i in range(res + 2):
            faces += [(p, p + 2, p + 8, p + 6), (p + 2, p + 3, p + 9, p + 8), (p + 4, p + 5, p + 11, p + 10),
                      ...
                      (p, p + 1, p + 7, p + 6)]
            p += 6
        faces += [(p2, p, p + 2, p2 + 2), (p2, p2 + 1, p + 1, p), (p + 1, p2 + 1, p2 + 5, p + 5),
                  (p2 + 5, p2 + 4, p + 4, p + 5),
                  (p2 + 2, p + 2, p + 3, p2 + 3)]
    ...
    def oval(width, height, res, jamb_w):
        ...
        for i in range(0, 2 * res - 2, 2):
            faces += [(i, i + 2, i + 3, i + 1), (i, i + p, i + p + 2, i + 2), (i + p, i + p + 1, i + p + 3, i + p + 2),
                      (i + p + 1, i + 1, i + 3, i + p + 3)]
        ...
        for i in range(res - 1):
            faces += [(p, p + 2, p + 8, p + 6), (p + 2, p + 3, p + 9, p + 8), (p + 4, p + 5, p + 11, p + 10),
                      ...
                      (p, p + 1, p + 7, p + 6)]
            p += 6
        faces += [(e + 1, p, p + 2, e + 3), (p + 2, p + 3, e + 4, e + 3), (p + 4, p + 5, e + 5, e + 6),
                  (p + 5, p + 1, e + 2, e + 6), (e + 1, p, p + 1, e + 2)]
    ...
    def polygon(radius, sides, jamb_w):
        ...
        for i in range(sides - 1):
            faces += [(p, p + 1, p + 3, p + 2), (p, p + 2, p + 2 + off, p + off),
                      (p + 1, p + 1 + off, p + 3 + off, p + 3),
                      (p + off, p + 2 + off, p + 3 + off, p + 1 + off)]
            p += 2
        faces += [(p, p + 1, 1, 0), (p, 0, off, p + off), (p + 1, p + 1 + off, 1 + off, 1),
                  (p + off, off, 1 + off, p + 1 + off)]
        ...
        p2 = p
        for i in range(sides - 1):
            faces += [(p, p + 6, p + 8, p + 2), (p + 2, p + 8, p + 9, p + 3), (p + 4, p + 10, p + 11, p + 5),
                      (p + 1, p + 5, p + 11, p + 7),
                      (p, p + 6, p + 7, p + 1)]
            p += 6
        faces += [(p2, p2 + 2, p + 2, p), (p2, p, p + 1, p2 + 1), (p + 1, p + 5, p2 + 5, p2 + 1),
                  (p2 + 5, p + 5, p + 4, p2 + 4),
                  (p2 + 2, p2 + 3, p + 3, p + 2)]
    ...
    def stationary(width, height, jamb_w, num):
        ...
        for i in range(num):
            ...
            create_rectangle_pane(verts, faces, glass_indices, width, height, sx, 0.75 / METRIC_INCH,
                                  (height + 2 * I) / 2,
                                  [0.0, 0.0, 0.0, 0.0])
    ...
    def create_window(context, types, sub_type, jamb_w, dh_width, dh_height, gang_num, gl_width, gl_height,
                      gl_slide_right,
                      so_width, so_height, so_height_tall, sides, radius, is_full_circle, angle, roundness, res,
                      is_slider,
